        ...
        if (!imagesLoaded) {
            try {
                ...
                List<MultipartFile> files = resources.stream()
                        .map(file -> {
                            try {
                                return new MockMultipartFile("file", file.getFilename(), "image/jpeg", file.getInputStream());
                            } catch (IOException e) {
                ...
                log.error("An unexpected error occurred while initializing image = {}", exception.getMessage());
            }
        }
    }
}
